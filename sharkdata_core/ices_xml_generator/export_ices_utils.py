...
class ExportStations():
    ...
    def get_mprog(self, station_name):
        ...
        mprog = sharkdata_core.TranslateValues().get_translated_value('monitoring_program_code', mprog)
        #
        return mprog
    ...
    def load_export_translate_values(self, resource_name):
        ...
        if resource:
            ...
            for index, row in enumerate(data_as_text.split('\n')):
                row = [item.strip() for item in row.split('\t')]
                if index == 0:
                    # Supposed to be at least 'internal_key', 'value', 'ices-xml'.
                    header = row
                else:
                    if len(row) >= 2:
        ...
        if resource:
            ...
            for index, row in enumerate(data_as_text.split('\n')):
                ...
                if index == 0:
                    ...
                    if len(row) >= 2:
                        row_dict = dict(zip(header, row))
                        self.translate_taxa_dict[row_dict.get('scientific_name', '')] = row_dict.get('aphia_id', '')
                ...
                if index == 0:
                    ...
                    if len(row) >= 2:
                        ...
                        self.translate_taxa_dict[row_dict.get('dyntaxa_scientific_name', '')] = \
                            (row_dict.get('helcom_peg_scientific_name', ''), row_dict.get('ices_rlist', ''))
