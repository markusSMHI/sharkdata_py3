...
class ExportIcesContent(object):
    ...
    def __init__(self, datarow_dict):
        ...
        self._translate_values = sharkdata_core.TranslateValues()
        self._export_stations = sharkdata_core.ExportStations()
        self._translate_taxa = sharkdata_core.TranslateTaxa()
        self._translate_taxa_to_helcom_peg = sharkdata_core.TranslateDyntaxaToHelcomPeg()
    ...
    def copy_to_ices_fields(self):
        ...
        self._dict['MYEAR-R00'] = self._get_value('visit_year', '').replace('-', '') # Format YYYYMMDD.
        self._dict['RFVER-R00'] = '3.2.5'
        
        # R90.SamplingPlatformRecord
        self._dict['SHIPC-R90'] = self._get_value('platform_code', '')
        self._dict['CRUIS-R90'] = self._get_value('', '')  # Will be generated in ices_expedition_key().
        self._dict['OWNER-R90'] = 'SWAM'
        self._dict['PRDAT-R90'] = str(datetime.date.today()).replace('-', '') # Format YYYYMMDD.
...
#             self._dict['STNNO-R91'] = stnno.replace('-', '') # '-' indicates error in DATSU.
        ...
        stnno = self._get_value('sampling_laboratory_code', '') + '-' + \
        ...
        self._dict['MPROG-R91'] = self._export_stations.get_mprog(self._get_value('station_name', ''))
        self._dict['WLTYP-R91'] = self._export_stations.get_wltyp(self._get_value('station_name', ''))
        self._dict['MSTAT-R91'] = self._export_stations.get_mstat(self._get_value('station_name', ''))
        self._dict['PURPM-R91'] = self._export_stations.get_purpm(self._get_value('station_name', ''))
...
#         self._dict['SMPNO-R34'] = self._get_value('sample_id', '')
        ...
        self._dict['TRSCS-R34'] = self._get_value('section_distance_start_m', '').replace(',', '.')
        ...
        self._dict['NPORT-R34'] = self._get_value('number_of_portions', '').replace('.', ',') # Note: Should be ','.
        ...
        self._dict['CPORT-R38'] = self._get_value('counted_portions', '').replace('.', ',') # Note: Should be ','.
    ...
    def cleanup_ices_fields(self):
        ...
        if not self._dict['ALABO-R21']:  
            self._dict['ALABO-R21'] = self._get_value('analytical_laboratory_name_en', '')
        if not self._dict['ALABO-R21']:  
            self._dict['ALABO-R21'] = self._get_value('analytical_laboratory_name_sv', '')
        #
...
#         self._dict['LONGI-R91'] = self.ices_lat_long(self._dict['LONGI-R91'])
        ...
        if self._dict['DTYPE-R34'] == 'PP':
            (scientific_name, rlist) = self.translate_scientific_name_to_helcom_peg(self._get_value('scientific_name', self._dict['SPECI-R38']))
            if scientific_name:
            ...
            # Other.
            (aphiaid, rlist) = self.translate_scientific_name_to_aphia_id(self._dict['SPECI-R38'])
            if aphiaid:
        ...
        if scientific_name in sharkdata_core.ExportFilter().get_filter_remove_list('scientific_name'):
            self._dict['SPECI-R38'] = '<REMOVE>' + self._dict['SPECI-R38'] # Is removed when generating XML file.
        ...
        if self._dict['DTYPE-R34'] == 'ZP':
            ...
            if size_min_um and size_max_um:
                self._dict['SIZCL-R38'] = size_min_um + '-' + size_max_um
                self._dict['SPECI-R38'] = '<REMOVE>' + self._dict['SPECI-R38'] # Is removed when generating XML file.
            elif size_min_um:
    ...
    def add_transect_fields(self):
        """ """
        transect_data_dict = sharkdata_core.TransectData().get_transect_data(self._dict)
        ...
        if (self._dict['DTYPE-R34'] == 'PB'): # or \
            ...
            if ('Skagerack' in self._get_value('dataset_file_name', '')) or \
               ('Skagerrak' in self._get_value('dataset_file_name', '')) or \
               ('Skagerrack' in self._get_value('dataset_file_name', '')) or \
               ('Skagerak' in self._get_value('dataset_file_name', '')):
                self._dict['TREDT-R40'] = 'CR' # TREDT.
                self._dict['TRSLN-R40'] = transect_data_dict.get('max_sample_max_depth_m','') # TRSLN. 
                self._dict['DEPAD-R40'] = 'N' # DEPAD.
                self._dict['SMTYP-R20'] = 'DIV-PHOT' # SMTYP.
                
                
                self._dict['LATRS-R40'] = self._get_value('sample_latitude_dd', '').replace(',', '.')
                self._dict['LNTRS-R40'] = self._get_value('sample_longitude_dd', '').replace(',', '.')
                self._dict['LATRE-R40'] = self._get_value('sample_latitude_dd', '').replace(',', '.')
                self._dict['LNTRE-R40'] = self._get_value('sample_longitude_dd', '').replace(',', '.')

                self._dict['TRSCS-R34'] = self._get_value('sample_min_depth_m', '').replace(',', '.')
                self._dict['TRSCE-R34'] = self._get_value('sample_max_depth_m', '').replace(',', '.')

                self._dict['TRCSD-R34'] = self._get_value('sample_min_depth_m', '').replace(',', '.')
                self._dict['TRCED-R34'] = self._get_value('sample_max_depth_m', '').replace(',', '.')
                ...
                self._dict['DEPAD-R40'] = 'N' # DEPAD.
            #
            stratum_id = sharkdata_core.TransectData().stratum_id(self._dict)
            ...
            if self._get_value('SAREA-R20') == '':
                ...
                if 'Kattegatt' in self._get_value('dataset_file_name', ''):
                    try:
                        section_end = float(self._get_value('section_distance_end_m').replace(',', '.'))
                        section_start = float(self._get_value('section_distance_start_m').replace(',', '.'))
                        #
...
#                               ' Date: ' + self._get_value('sample_date'))
                ...
                if  ('_DEEP_' in self._get_value('dataset_file_name')) or \
                    ...
                    try:
                        section_end = float(self._get_value('section_distance_end_m').replace(',', '.'))
                        section_start = float(self._get_value('section_distance_start_m').replace(',', '.'))
                        transect_width = float(self._get_value('transect_width_m').replace(',', '.'))
                        #
                        ...
                        try:
                            print('ICES: Failed to calculate SAREA for' + 
                                  '  Station: ' + self._get_value('station_name') +
                                  '  Date: ' + str(self._get_value('sample_date')) +
                                  '  Section distance end m: ' + str(self._get_value('section_distance_end_m')) +
                                  '  Section distance start m: ' + str(self._get_value('section_distance_start_m')) +
                                  '  Transect width m: ' + str(self._get_value('transect_width_m')) 
                                  )
                        except:
                            pass
                    #
...
#                 print('SAREA=0: Parameter: ' + self._get_value('parameter') + '   Dataset: ' + self._get_value('dataset_name'))
            ...
            try:
                if section_start_depth_m:
                    section_start_depth_m = float(section_start_depth_m.replace(',', '.'))
        ...
        for ices_field_name in ['TREDP-R40', 'TRCSD-R34', 'TRCED-R34', 'MNDEP-R38', 'MXDEP-R38']:
            ...
            try:
                if depth_s:
                    ...
                    if depth_s < 0.0:
                        ...
                        print('DEBUG: Depth value set to zero: ' + ices_field_name + '   Value: ' + str(depth_s))
            except:
                print('DEBUG: Failed to convert to float. ' + ices_field_name + '   Value: ' + str(depth_s))
                pass
...
#         self.rec20_fields = ['SLABO', 'SMLNK', 'SMTYP', 'MESHS', 'SAREA', 'LNSMB']
    ...
    def _get_value(self, internal_key, default=''):
        """ """
        translate_values = sharkdata_core.TranslateValues()
        #
    ...
    def translate_scientific_name_to_helcom_peg(self, scientific_name):
        """ """
        (helcom_peg_scientific_name, rlist) = self._translate_taxa_to_helcom_peg.get_translated_taxa_and_rlist(scientific_name)
        #
    ...
    def ices_expedition_key(self):
        ...
        key_list.append(self._dict.get('RLABO-R00', '')) # visit.getParent().getReporting_institute_code()
        key_list.append(self._dict.get('DTYPE-R34', '')) # IcesUtil.convertToIcesDatatype(visit
        # Part 2.
    ...
    def ices_parameter_name(self, value):
        ...
        elif value == 'Biovolume concentration': return 'BMCEVOL' # PP: Biovolume concentration, mm3/l, BMCEVOL.
        ...
        if unit_name == 'nr/l': return 'nr/dm3'
            ...
            if self._dict['parameter'] == 'Wet weight':
                try:
                    ...
                    print('DEBUG: Failed to divide value by 1000: ' + self._dict.get('VALUE-R38', ''))
