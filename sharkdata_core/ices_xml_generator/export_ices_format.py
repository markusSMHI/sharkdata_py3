...
class IcesXmlGenerator(object):
    ...
    def add_row(self, datarow_dict, ):
        ...
        if datarow_dict['parameter'] in self.filter_parameters:
            #
            ices_content = sharkdata_core.ExportIcesContent(datarow_dict)
            ...
            if key38 in self.datarow_dict:
                ...
                if (datarow_dict['DTYPE-R34'] == 'ZP') and \
                    ...
                    try:
                        ...
                        datarow_dict['VALUE-R38'] = '{0:f}'.format(aggregated_value) # .rename(',', '.')
                    except:
                        print('DEBUG: Failed to aggregate ZP values.')
                else:
                    if settings.DEBUG:
                        self._duplicate_counter += 1
                        print('DEBUG: Duplicate found (' + str(self._duplicate_counter) + '): ' + key38)
            #
            self.datarow_dict[key38] = datarow_dict

            # ZP must have both ABUNDNR and WETWT.
            ices_content.check_zp_abundnr_wetwt(self.datarow_dict, self._zp_abundnr_wetwt_list)
...
#             if settings.DEBUG: print('DEBUG: Row as dict: ' + str(rowdict))
            ...
            if rec00_element == False:
                ...
                for field in self.rec00_fields:
                    if rowdict.get(field, False):
                        ...
                        out_rows.append('  <' + field_ices + '>' + rowdict.get(field, '') + '</' + field_ices + '>')
            ...
            if rec34_lastusedkey != rowdict.get('rec34_key', ''):
                ...
                if rowdict.get('SMPNO-R34', '').startswith('9999'):
                    ...
                    rowdict['SMPNO-R34'] = rowdict['SMPNO-R34'] + '-' + str(self._34rec_counter)
                #
            ...
            for key in self._rec20_dict.keys():
                ...
                for field in self.rec20_fields:
                    if r20_dict.get(field, False):
                        field_ices = field.split('-R')[0]
                        out_rows.append('    <' + field_ices + '>' + r20_dict.get(field, '') + '</' + field_ices + '>')
                out_rows.append('  </R20.SamplingMethod>')
        #    
            ...
            for key in self._rec21_dict.keys():
                ...
                for field in self.rec21_fields:
                    if r21_dict.get(field, False):
                        field_ices = field.split('-R')[0]
                        out_rows.append('    <' + field_ices + '>' + r21_dict.get(field, '') + '</' + field_ices + '>')
                out_rows.append('  </R21.AnalyticalMethod>')
        #    
        if rec00_element:
            out_rows.append('</R00.FileInformation>')
    ...
    def define_fields_and_keys(self):
        ...
        self.filter_parameters = ['# counted', 
                                  'Wet weight', 
                                  'Biovolume concentration', 
                                  'Carbon concentration', 
                                  'Cover (%)', ]
        # ICES fields definitions.
        self.rec00_fields = ['RLABO-R00', 'CNTRY-R00', 'MYEAR-R00', 'RFVER-R00', ]
        self.rec90_fields = ['SHIPC-R90', 'CRUIS-R90', 'OWNER-R90', 'PRDAT-R90', ]
        self.rec91_fields = ['STNNO-R91', 'LATIT-R91', 'LONGI-R91', 'POSYS-R91', 'SDATE-R91', 'STIME-R91', 'ETIME-R91', 
                             'WADEP-R91', 'STATN-R91', 'MPROG-R91', 'WLTYP-R91', 'MSTAT-R91', 'PURPM-R91', 'EDATE-R91', ]
        # self.rec92_fields = ['RSRVD-R92', 'MATRX-R92', 'PARAM-R92', 'MUNIT-R92', 'VALUE']
        self.rec40_fields = ['TRANS-R40', 'TRDGR-R40', 'POSYS-R40', 'LATRS-R40', 'LNTRS-R40', 'TRSLN-R40', 'TREDT-R40', 
                             'LATRE-R40', 'LNTRE-R40', 'DEPAD-R40', 'TREDP-R40', 'MXVEG-R40', 'SPVEG-R40', 'R40-RLIST-NOT_USED-R40', ]
        self.rec34_fields = ['DTYPE-R34', 'TRANS-R34', 'SMPNO-R34', 'SMLNK-R20', 'ATIME-R34', 'NOAGG-R34', 'FNFLA-R34', 'FINFL-R34', 
                             'SMVOL-R34', 'WIRAN-R34', 'CLMET-R34', 'FLVOL-R34', 'SUBST-R34', 'DEPOS-R34', 'PCNAP-R34', 'PRSUB-R34', 
                             'TRSCS-R34', 'TRSCE-R34', 'NPORT-R34', 'TRCSD-R34', 'TRCED-R34', ]
        self.rec38_fields = ['MNDEP-R38', 'MXDEP-R38', 'SPECI-R38', 'RLIST-R38', 'SFLAG-R38', 'STRID-R38', 'SIZCL-R38', 'RSRVD-R38', 
                             'MAGNI-R38', 'COEFF-R38', 'TRPHY-R38', 'STAGE-R38', 'SEXCO-R38', 'PARAM-R38', 'MUNIT-R38', 
                             'VFLAG-R38', 'QFLAG-R38', 'VALUE-R38', 'CPORT-R38', 'SDVOL-R38', 'AMLNK-R21', ]
        self.rec20_fields = ['SLABO-R20', 'SMLNK-R20', 'SMTYP-R20', 'NETOP-R20', 'MESHS-R20', 'SAREA-R20', 'LNSMB-R20', 'SPEED-R20', 
                             'PDMET-R20', 'SPLIT-R20', 'OBSHT-R20', 'DURAT-R20', 'DUREX-R20', 'ESTFR-R20', ]
        self.rec21_fields = ['AMLNK-R21', 'ALABO-R21', 'METDC-R21', 'REFSK-R21', 'METST-R21', 'METFP-R21', 'METPT-R21', 'METCX-R21', 
                             'METPS-R21', 'METOA-R21', 'AGDET-R21', 'SREFW-R21', 'SPECI-R21', 'RLIST-R21', 'ORGSP-R21', 'SIZRF-R21', 
                             'FORML-R21', 'ACCRD-R21', 'ACORG-R21', ]
        self.rec94_fields = ['ICLNK-R94', 'ICCOD-R94', 'ICLAB-R94', ]
        
        # Key definitions.
        self.rec00_keys = []
        self.rec90_keys = ['CRUIS-R90',]
        self.rec91_keys = ['CRUIS-R90', 'STNNO-R91', 'SDATE-R91', 'STIME-R91',]
        # self.rec92_keys = ['CRUIS-R90', 'STNNO-R91', ]
        self.rec40_keys = ['CRUIS-R90', 'STNNO-R91', 'SDATE-R91', 'STIME-R91', 'TRANS-R40', ]
        self.rec34_keys = ['CRUIS-R90', 'STNNO-R91', 'SDATE-R91', 'STIME-R91', 'TRANS-R40', 'SMPNO-R34',
                           'TRSCS-R34', 'TRSCE-R34', 'TRCSD-R34', 'TRCED-R34', 
                           ]
        self.rec38_keys = ['CRUIS-R90', 'STNNO-R91', 'SDATE-R91', 'STIME-R91', 'TRANS-R40', 'SMPNO-R34',
                           'TRSCS-R34', 'TRSCE-R34', 'TRCSD-R34', 'TRCED-R34', 
                           'STRID-R38', 
                           'SPECI-R38', 'SIZCL-R38', 
                           'STAGE-R38', 'SEXCO-R38', 'PARAM-R38', 'MUNIT-R38', ]
        self.rec20_keys = ['SMLNK-R20', ]
        self.rec21_keys = ['AMLNK-R21', ]
        self.rec94_keys = ['AMLNK-R21', 'ICLNK-R94', ]
