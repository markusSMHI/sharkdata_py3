...
def listCtdProfiles(request):
    ...
    years = models.CtdProfiles.objects.values_list('visit_year', flat=True).distinct().order_by('-visit_year')
    projects = models.CtdProfiles.objects.values_list('sample_project_code', flat=True).distinct().order_by('sample_project_code')
    platforms = models.CtdProfiles.objects.values_list('platform_code', flat=True).distinct().order_by('platform_code')
    ...
    try: selected_latitude_from = float( urllib.parse.unquote_plus(selected_latitude_from)) 
    except: pass
    try: selected_latitude_to = float( urllib.parse.unquote_plus(selected_latitude_to))
    except: pass
    try: selected_longitude_from = float( urllib.parse.unquote_plus(selected_longitude_from)) 
    except: pass
    try: selected_longitude_to = float( urllib.parse.unquote_plus(selected_longitude_to))
    except: pass


    db_filter_dict = {}
    if selected_year not in ['', 'All', 'selected="selected"']:
        db_filter_dict['{0}__{1}'.format('visit_year', 'iexact')] = urllib.parse.unquote_plus(selected_year)
    if selected_project not in ['', 'All']:
        db_filter_dict['{0}__{1}'.format('sample_project_code', 'iexact')] = urllib.parse.unquote_plus(selected_project)
    if selected_platform not in ['', 'All']:
        db_filter_dict['{0}__{1}'.format('platform_code', 'iexact')] = urllib.parse.unquote_plus(selected_platform)
    if selected_date not in ['', ]:
        db_filter_dict['{0}__{1}'.format('sample_date', 'iexact')] = urllib.parse.unquote_plus(selected_date)
...
#         datasets = models.Datasets.objects.all().order_by('dataset_name')
    ...
    ctdprofiles = models.CtdProfiles.objects.filter(**db_filter_dict).order_by('visit_year', 'platform_code', 'visit_id')
